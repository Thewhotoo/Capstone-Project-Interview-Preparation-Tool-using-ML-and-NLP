"""
A1-on-V4 analysis — READ-ONLY. Consumes `../v4_diagnostic_58.jsonl` (gold
labels + rationale metadata only), `a1_predictions_on_v4.json` (just
produced), and `../v3_predictions_on_v4.json` (A0/V3's existing, untouched
predictions) to build the A1 report AND the direct A0-vs-A1 comparison.
Performs no inference itself, trains nothing, and does not modify any A0/V3
artifact. Writes exactly one new file: `a1_on_v4_report.json`.
"""
...
A1_PREDICTIONS_PATH = os.path.join(_HERE, "a1_predictions_on_v4.json")
A0_PREDICTIONS_PATH = os.path.join(_V4_DIR, "v3_predictions_on_v4.json")  # existing, untouched
REPORT_PATH = os.path.join(_HERE, "a1_on_v4_report.json")
...
def main():
    ...
    # group-by-group A0 vs A1 comparison
    group_comparison = []
    for p in a1_pair_results:
        ...
        group_comparison.append({
            ...
            "a1_predicted_values": p["predicted_values"], "a1_status": p["status"],
            "status_changed": a0p["status"] != p["status"],
        })

    # per-category summaries for both, for the comparison table
    ...
    # ---- critical failure checks (A1), mirroring analyze_v3_on_v4.py ----
    checks = {}
    detailed_irrelevant = [r for r in a1_preds if r["diagnostic_category"] == "relevance_alignment"
    ...
    concise_relevant = [r for r in a1_preds if r["diagnostic_category"] == "relevance_alignment"
    ...
    misconceptions = [r for r in a1_preds if r["diagnostic_category"] == "technical_correctness"
    ...
    partial_multipart = [r for r in a1_preds if r["diagnostic_category"] == "multipart_completeness"
    ...
    report = {
        "n_examples_evaluated": len(a1_preds),
        "a1_aggregate_metrics": a1_aggregate,
        "a0_aggregate_metrics_for_reference": a0_aggregate,
        "a1_category_metrics": a1_category_metrics,
        "a1_pair_summary_overall": a1_pair_summary_overall,
        "a0_pair_summary_overall_for_reference": a0_pair_summary_overall,
        "relevance_alignment_pair_summary": {"a0": a0_pair_summary_rel, "a1": a1_pair_summary_rel},
        "category_a0_vs_a1_table": category_summary_table,
        "group_by_group_a0_vs_a1": group_comparison,
        "a1_critical_failure_checks": checks,
    }
