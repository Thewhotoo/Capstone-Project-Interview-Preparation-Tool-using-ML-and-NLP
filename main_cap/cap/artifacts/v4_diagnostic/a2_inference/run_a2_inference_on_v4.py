"""
A2-checkpoint-on-V4 inference runner — READ-ONLY, INFERENCE-ONLY.

Identical procedure to `../run_v3_inference_on_v4.py` (V3/A0's inference
script) and `../a1_inference/run_a1_inference_on_v4.py` (A1's), pointed at
the A2 checkpoint instead -- same production input-construction functions
...
same max_length, same dimension ordering. The ONLY variable across this
script and A1's is which checkpoint file is loaded and where the output is
written -- everything else is byte-for-byte the same procedure, so A2's V4
results are directly comparable to A0's and A1's.
...
- modify `v4_diagnostic_58.jsonl` or any V1/V2/V3/A0/A1 artifact -- writes
  exactly one new file, `a2_predictions_on_v4.json`, into this isolated
  `a2_inference/` subdirectory.
"""
...
OUTPUT_PATH = os.path.join(_HERE, "a2_predictions_on_v4.json")
...
def main() -> None:
    ...
    print(f"A2 checkpoint found: {CHECKPOINT_PATH} ({size_mb:.1f} MB)")
    ...
    print("Reconstructing MultiTaskModel architecture (4 canonical CORAL heads) and loading A2 checkpoint weights...")
