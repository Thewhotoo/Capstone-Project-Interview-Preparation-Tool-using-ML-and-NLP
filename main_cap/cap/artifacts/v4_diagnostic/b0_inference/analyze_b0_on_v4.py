"""
B0-on-V4 analysis — READ-ONLY. Consumes `../v4_diagnostic_58.jsonl` (gold
labels + rationale metadata only), `b0_predictions_on_v4.json` (just
produced), and the existing, untouched `../v3_predictions_on_v4.json` (A0),
`../a1_inference/a1_predictions_on_v4.json` (A1), and
`../a2_inference/a2_predictions_on_v4.json` (A2) to build the B0 report AND
the direct A0-vs-B0 architecture-only comparison (B0's primary comparison,
per the V6 Ablation Design Review §20 -- B0 uses A0's exact unweighted
loss, so A0-vs-B0 isolates architecture as the sole variable), with A1/A2
kept for reference to show what loss weighting alone achieved. Performs no
inference itself, trains nothing, and does not modify any A0/V3/A1/A2
artifact. Writes exactly one new file: `b0_on_v4_report.json`.
"""
...
B0_PREDICTIONS_PATH = os.path.join(_HERE, "b0_predictions_on_v4.json")
...
A2_PREDICTIONS_PATH = os.path.join(_V4_DIR, "a2_inference", "a2_predictions_on_v4.json")  # existing, untouched
REPORT_PATH = os.path.join(_HERE, "b0_on_v4_report.json")
...
def main():
    ...
    with open(B0_PREDICTIONS_PATH, encoding="utf-8") as f:
        b0_preds = json.load(f)
    with open(A0_PREDICTIONS_PATH, encoding="utf-8") as f:
        a0_preds = json.load(f)
    assert len(b0_preds) == 58
    assert len(a0_preds) == 58

    # A1/A2 are kept for reference only (may be absent in some environments,
    # e.g. before A2's inference has been run there -- handled gracefully).
    a1_preds = _load_if_present(A1_PREDICTIONS_PATH)
    a2_preds = _load_if_present(A2_PREDICTIONS_PATH)

    # ---- aggregate + per-category metrics (B0) ----
    b0_aggregate = {dim: dim_metrics(b0_preds, dim) for dim in DIMS}
    a0_aggregate = {dim: dim_metrics(a0_preds, dim) for dim in DIMS}
    a1_aggregate = {dim: dim_metrics(a1_preds, dim) for dim in DIMS} if a1_preds else None
    a2_aggregate = {dim: dim_metrics(a2_preds, dim) for dim in DIMS} if a2_preds else None

    by_category_b0 = {}
    for row in b0_preds:
        by_category_b0.setdefault(row["diagnostic_category"], []).append(row)
    b0_category_metrics = {
        cat: {"n_examples": len(rows), "per_dimension": {dim: dim_metrics(rows, dim) for dim in DIMS}}
        for cat, rows in by_category_b0.items()
    }

    # ---- pairwise diagnostic (B0) ----
    b0_pair_results = pair_analysis(b0_preds)
    a0_pair_results = pair_analysis(a0_preds)
    a1_pair_results = pair_analysis(a1_preds) if a1_preds else None
    a2_pair_results = pair_analysis(a2_preds) if a2_preds else None
    a0_pair_by_id = {p["pair_group_id"]: p for p in a0_pair_results}

    b0_pair_summary_overall = summarize_pairs(b0_pair_results)
    a0_pair_summary_overall = summarize_pairs(a0_pair_results)
    a1_pair_summary_overall = summarize_pairs(a1_pair_results) if a1_pair_results else None
    a2_pair_summary_overall = summarize_pairs(a2_pair_results) if a2_pair_results else None
    b0_pair_summary_rel = summarize_pairs(b0_pair_results, "relevance_alignment")
    a0_pair_summary_rel = summarize_pairs(a0_pair_results, "relevance_alignment")
    a1_pair_summary_rel = summarize_pairs(a1_pair_results, "relevance_alignment") if a1_pair_results else None
    a2_pair_summary_rel = summarize_pairs(a2_pair_results, "relevance_alignment") if a2_pair_results else None

    # group-by-group A0-vs-B0 comparison (primary: architecture-only, same
    # unweighted loss) -- A1/A2 predicted values included for reference
    # when available.
    a1_pair_by_id = {p["pair_group_id"]: p for p in a1_pair_results} if a1_pair_results else {}
    a2_pair_by_id = {p["pair_group_id"]: p for p in a2_pair_results} if a2_pair_results else {}
    group_comparison = []
    for p in b0_pair_results:
        a0p = a0_pair_by_id[p["pair_group_id"]]
        a1p = a1_pair_by_id.get(p["pair_group_id"])
        a2p = a2_pair_by_id.get(p["pair_group_id"])
        group_comparison.append({
            ...
            "a1_predicted_values": a1p["predicted_values"] if a1p else None,
            "a1_status": a1p["status"] if a1p else None,
            "a2_predicted_values": a2p["predicted_values"] if a2p else None,
            "a2_status": a2p["status"] if a2p else None,
            "b0_predicted_values": p["predicted_values"], "b0_status": p["status"],
            "status_changed_a0_to_b0": a0p["status"] != p["status"],
        })

    # per-category summaries, for the comparison table
    ...
    for cat in ("relevance_alignment", "multipart_completeness", "technical_correctness",
        ...
        a1_rows = [r for r in a1_preds if r["diagnostic_category"] == cat] if a1_preds else None
        a2_rows = [r for r in a2_preds if r["diagnostic_category"] == cat] if a2_preds else None
        b0_rows = [r for r in b0_preds if r["diagnostic_category"] == cat]
        a0_dim_m = dim_metrics(a0_rows, target_dim) if target_dim else None
        a1_dim_m = dim_metrics(a1_rows, target_dim) if (target_dim and a1_rows is not None) else None
        a2_dim_m = dim_metrics(a2_rows, target_dim) if (target_dim and a2_rows is not None) else None
        b0_dim_m = dim_metrics(b0_rows, target_dim) if target_dim else None
        category_summary_table[cat] = {
            ...
            "b0_qwk": b0_dim_m["qwk"] if b0_dim_m else None,
            "a0_pair_summary": summarize_pairs(a0_pair_results, cat),
            "a1_pair_summary": summarize_pairs(a1_pair_results, cat) if a1_pair_results else None,
            "a2_pair_summary": summarize_pairs(a2_pair_results, cat) if a2_pair_results else None,
            "b0_pair_summary": summarize_pairs(b0_pair_results, cat),
        }

    # ---- critical failure checks (B0), mirroring analyze_v3_on_v4.py / analyze_a1_on_v4.py / analyze_a2_on_v4.py ----
    checks = {}
    detailed_irrelevant = [r for r in b0_preds if r["diagnostic_category"] == "relevance_alignment"
    ...
    concise_relevant = [r for r in b0_preds if r["diagnostic_category"] == "relevance_alignment"
    ...
    misconceptions = [r for r in b0_preds if r["diagnostic_category"] == "technical_correctness"
    ...
    partial_multipart = [r for r in b0_preds if r["diagnostic_category"] == "multipart_completeness"
    ...
    report = {
        "n_examples_evaluated": len(b0_preds),
        "b0_aggregate_metrics": b0_aggregate,
        "a0_aggregate_metrics_for_reference": a0_aggregate,
        "a1_aggregate_metrics_for_reference": a1_aggregate,
        "a2_aggregate_metrics_for_reference": a2_aggregate,
        "b0_category_metrics": b0_category_metrics,
        "b0_pair_summary_overall": b0_pair_summary_overall,
        ...
        "a2_pair_summary_overall_for_reference": a2_pair_summary_overall,
        "relevance_alignment_pair_summary": {
            "a0": a0_pair_summary_rel, "a1": a1_pair_summary_rel, "a2": a2_pair_summary_rel, "b0": b0_pair_summary_rel,
        },
        "category_a0_vs_a1_vs_a2_vs_b0_table": category_summary_table,
        "group_by_group_a0_vs_a1_vs_a2_vs_b0": group_comparison,
        "b0_critical_failure_checks": checks,
        "primary_comparison_note": (
            "B0's primary comparison is A0-vs-B0 (both unweighted loss; the "
            "ONLY variable is architecture -- private per-dimension MLP vs. "
            "none). A1/A2 are included for reference only, to show what loss "
            "weighting alone achieved on the same V4 benchmark -- see "
            "docs/architecture/V6_Experiment_B_Design_Review.md §20."
        ),
    }
