"""
SANITY CHECK ONLY -- not part of the V4 deliverable, not saved as an output
artifact (prints to stdout only, writes nothing). Confirms the checkpoint at
`artifacts/four_dim_training_v3/best_checkpoint_weights.pt` actually
reproduces the REPORTED V3 test-set QWK numbers before trusting it for V4
inference. Uses the frozen V2 split's real 27-example test set (same split
V3 was evaluated on) via the existing, unmodified
`four_dim_experiment_split.py` / `four_dim_experiment_v2_split.py` loaders.
...
_CAP_DIR = os.path.dirname(os.path.dirname(_HERE))
...
def main():
    ...
    print("\n--- Reproduced V3 test metrics (should closely match reported: TC 0.1037, Depth 0.8002, Rel 0.1690, Grounding 0.5810, mean 0.4135) ---")
    ks = []
    for dim in CANONICAL_DIMENSION_KEYS:
