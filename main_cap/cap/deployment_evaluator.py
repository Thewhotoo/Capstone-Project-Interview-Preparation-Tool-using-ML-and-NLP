...
live application's evaluator registry, with a three-tier fallback chain
that never crashes the application.

V3 SINGLE-OVERALL-SCORE CUTOVER (this session, additive): the PRIMARY
deployment target is now `deployed_model_overall_single_v3/` --
`overall_single_evaluator.OverallSingleEvaluator`, ONE shared
`CoralOrdinalHead` (`overall_score_model.OverallScoreModel`) predicting a
single learned overall 0-4 score, paired with
`heuristic_diagnostics.HeuristicDiagnosticsEngine`'s four fully
...
`overall_single_evaluator.py`'s module docstring for the full
architecture and the enforced "heuristics never touch overall_score"
guarantee.

THREE-TIER FALLBACK CHAIN (explicit deployment decision, in priority order):
  1. `deployed_model_overall_single_v3/` -- the new evaluator, tried first.
  2. `deployed_model_a2/` -- the PREVIOUS production evaluator (four-
     dimension A2, `HybridEvaluator`-wrapped exactly as it was before this
     session), tried if and only if step 1 fails for any reason. This is
     A2's explicit, always-reachable ROLLBACK PATH -- nothing about A2's
     own loading/wiring logic changed; `_try_activate_a2()` below is the
     SAME code `bootstrap_production_evaluator()` used to run
     unconditionally, now factored into its own function and demoted to
     "the rollback tier" rather than removed.
  3. A bare `HeuristicEvaluator` -- tried if and only if both 1 and 2 fail.
     Always succeeds; this is the same, unconditional final fallback this
     module has always had.
`activate_a2_rollback()` (below) additionally exposes tier 2+3 as a
STANDALONE, directly callable function -- an explicit way to force A2 (or
heuristic-only) live without going through tier 1 at all, for a manual
rollback decision, without needing to touch `deployed_model_overall_single_v3/`
on disk.

Neither `deployed_model_a2/` nor `deployed_model/` (the legacy 12-dimension
`experiment_4` checkpoint) is modified, moved, or deleted by this cutover --
both remain byte-for-byte exactly as they were. `LEGACY_DEPLOYED_MODEL_DIR`
documents where the legacy deployment still lives (not read by this
module's bootstrap logic, kept purely as a rollback pointer, unchanged from
before this session).

ARCHITECTURE MISMATCH, HANDLED DELIBERATELY: the new evaluator's checkpoint
is `overall_score_model.OverallScoreModel` (one shared `CoralOrdinalHead`),
NOT `model_heads.MultiTaskModel`'s per-dimension `DimensionOrdinalHeads`
shape A2/the legacy deployment use -- so it is loaded via
...
# ── Tier 1: V3 single-overall-score -- the PRIMARY deployment target ────────
...
# ── Tier 2: A2 (four-dimension, canonical) -- the ROLLBACK deployment target ─
...
def _try_activate_overall_single_v3() -> bool:
    """Tier 1. Returns True and makes `OverallSingleEvaluator` active iff
    every step succeeds; returns False (making NO registry change) on any
    failure -- missing files, a corrupt/mismatched checkpoint, or a
    `PromotionDecision` that isn't `approved`. Never raises."""
    try:
...
def _try_activate_a2() -> bool:
    """Tier 2 / A2 rollback. Identical logic to this module's pre-V3-cutover
...
def _activate_heuristic_fallback() -> None:
    """Tier 3. Always succeeds -- the final, unconditional fallback."""
...
def bootstrap_production_evaluator() -> None:
    """
    Tries the three-tier fallback chain in order (see module docstring):
    V3 single-overall-score -> A2 -> bare HeuristicEvaluator. This function
    never raises; a broken/missing deployment at any tier must never crash
    the application.
    """
...
def activate_a2_rollback() -> None:
    ...
    attempting the V3 single-overall-score tier at all -- callable directly
    by an operator/ops script to force a rollback decision, independent of
    whatever is or isn't present on disk at
    `deployed_model_overall_single_v3/`. Never raises.
    """
    if _try_activate_a2():
        return
    _activate_heuristic_fallback()
