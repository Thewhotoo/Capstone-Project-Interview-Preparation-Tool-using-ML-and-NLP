...
#
# Resume Discussion is not a Technical Interview: it only ever discusses
# things the candidate actually claims to have done. Every unit below must
# resolve to exactly one real entry in the Candidate Profile — a project, an
# experience entry, a certification, or a skill used *within* one of those —
# never a standalone computer-science subject. Priority order matches the
# product spec exactly:
#   1. project_deep_dive   — a project's own interview_seeds (Gemini-authored,
#                             already project-scoped)
#   2. project_overview    — "what did you build" for each project
#   3. experience          — internship/work entries
#   4. certification       — certifications actually listed
#   5. skill_in_context     — a technical_topics entry, ONLY if it cites a real
#                             project or experience entry; rejected otherwise
# Nothing here treats `technical_topics` as independent interview subjects —
# see `_build`'s skill_in_context section for the traceability check that
# enforces this.
...
class TopicPool:
    ...
    Profile. Nothing about ordering is fixed upfront: `select_next` is called
    every time the decision policy says "move on," and picks from whatever's
    still `unasked` — but always within the strict priority hierarchy above,
    never a precomputed sequence and never a topic that isn't traceable back
    to one specific Candidate Profile entry.
    """

    def __init__(self, profile: dict):
        self.units: dict[str, dict] = {}
        self.rejected: list[dict] = []  # untraceable technical_topics, kept for diagnostics/tests
        self.recent_styles: list[str] = []  # last few question-phrasing styles used, any category — anti-repetition
        self._build(profile)

    def _add(self, category: str, text_seed: str | None, grounding: dict | None,
              source_type: str, source_id: str, source_field: str, reason: str,
              priority_boost: bool = False) -> str:
        unit_id = f"topic_{len(self.units)}"
        self.units[unit_id] = {
            "id": unit_id,
            "category": category,
            "text_seed": text_seed,
            ...
            # Traceability — every unit must resolve to exactly one Candidate
            # Profile entry; these three fields are that trace.
            "source_type": source_type,     # "project" | "experience" | "certification"
            "source_id": source_id,         # the project title / "role at company" / cert name
            "source_field": source_field,   # which Candidate Profile field produced this
            "reason": reason,               # human-readable justification, for logging/testing
        }
        return unit_id
    ...
    def _build(self, profile: dict):
        ...
        def find_project_by_title(title: str) -> dict | None:
            title_lower = (title or "").strip().lower()
            if not title_lower:
                return None
            for p in projects:
                if p.get("title", "").strip().lower() == title_lower:
                    return p
            return None

        def find_experience_by_label(label: str) -> dict | None:
            label_lower = (label or "").strip().lower()
            if not label_lower:
                return None
            for e in experiences:
                role = e.get("role", "").strip().lower()
                company = e.get("company", "").strip().lower()
                combo = f"{role} at {company}".strip()
                if label_lower in (role, combo) or (role and role in label_lower):
                    return e
            return None
        ...
        for project in projects:
            ...
            for seed in project.get("interview_seeds", []):
                self._add(
                    "project_deep_dive", seed, {"project": project},
                    source_type="project", source_id=title, source_field="interview_seeds",
                    reason=f"projects[title={title!r}].interview_seeds",
                    priority_boost=touches_weakness(title) or touches_weakness(seed),
                )

        # ── Priority 2: project implementation details (overview) ──────
        for project in projects:
            title = project.get("title", "")
            if not title:
                continue
            self._add(
                "project_overview", None, {"project": project},
                source_type="project", source_id=title, source_field="summary",
                reason=f"projects[title={title!r}].summary/technologies",
                priority_boost=touches_weakness(title),
            )

        # ── Priority 3: internship / work experience ────────────────────
        for exp in experiences:
            ...
            self._add(
                "experience", None, {"experience": exp},
                source_type="experience", source_id=label, source_field="description",
        ...
        for cert in profile.get("certifications", []):
            ...
            if not name:
                continue
            cert_dict = cert if isinstance(cert, dict) else {"name": name}
            self._add(
                "certification", None, {"certification": cert_dict},
                source_type="certification", source_id=name, source_field="name",
                reason=f"certifications[name={name!r}]",
            )

        # ── Priority 5: skills, only in the context of where they were
        # used. A technical_topics entry is only ever asked about if it
        # resolves to a real project or experience entry already on the
        # profile — otherwise it's a technology floating free of any claim,
        # which is exactly what Resume Discussion must never ask about.
        for topic_entry in blueprint.get("technical_topics", []):
            topic = topic_entry.get("topic", "") if isinstance(topic_entry, dict) else ""
            if not topic:
                continue
            origin_project = topic_entry.get("originating_project", "") if isinstance(topic_entry, dict) else ""
            origin_experience = topic_entry.get("originating_experience", "") if isinstance(topic_entry, dict) else ""
            evidence = topic_entry.get("evidence", "") if isinstance(topic_entry, dict) else ""

            matched_project = find_project_by_title(origin_project) if origin_project else None
            matched_experience = None if matched_project else (
                find_experience_by_label(origin_experience) if origin_experience else None
            )

            if matched_project:
                title = matched_project.get("title", "")
                self._add(
                    "skill_in_context", topic, {"project": matched_project},
                    source_type="project", source_id=title, source_field="technical_topics",
                    reason=f"technical_topics: {topic!r} (evidence: {evidence!r})",
                    priority_boost=touches_weakness(topic) or touches_weakness(title),
                )
            elif matched_experience:
                role = matched_experience.get("role", "")
                company = matched_experience.get("company", "")
                label = f"{role} at {company}" if company else role
                self._add(
                    "skill_in_context", topic, {"experience": matched_experience},
                    source_type="experience", source_id=label, source_field="technical_topics",
                    reason=f"technical_topics: {topic!r} (evidence: {evidence!r})",
                    priority_boost=touches_weakness(topic),
                )
            else:
                # Cannot trace to a real project or experience entry on this
                # profile — reject rather than ask a free-floating tech
                # question. Kept in `self.rejected` for diagnostics/tests.
                self.rejected.append({
                    "topic": topic, "originating_project": origin_project,
                ...
                logger.info(
                    ...
                    "originating_experience=%r) — no match in this Candidate Profile",
                    topic, origin_project, origin_experience,
                )

    def categories_present(self) -> set[str]:
        """Which categories actually have units on this profile (e.g. a
        profile with no certifications simply never has that category)."""
        return {u["category"] for u in self.units.values()}

    def categories_covered(self) -> set[str]:
        """Categories with at least one unit that's been asked (active,
        covered, or skipped) — i.e. touched at least once this session."""
        return {u["category"] for u in self.units.values() if u["status"] != "unasked"}

    def all_categories_covered(self) -> bool:
        """True once every category present on this profile has been
        touched at least once — the coverage guarantee the session's
        completion check enforces before it's allowed to end."""
        return self.categories_present() <= self.categories_covered()

    def select_next(self, last_category: str | None) -> dict | None:
        """
        Pick the highest-priority `unasked` unit. The dominant factor is the
        hierarchy tier (project seeds > project overview > experience >
        certification > skill-in-context) — never FIFO within a tier — with
        one override: a category that hasn't been touched at all yet gets a
        large coverage bonus, so a project with many interview_seeds can't
        consume the entire question budget before experience/certifications
        are ever reached. In practice this produces one natural sweep through
        every present category first (still in tier order, since the bonus
        is equal across untouched categories), then normal tier-priority
        depth for the remaining budget. Weak-area topics and category
        diversity break ties within a tier, plus a small random component so
        repeated sessions on the same profile don't feel scripted. Every call
        logs the winner and why every other unasked candidate lost, for
        diagnostics/verification.
        """
        candidates = [u for u in self.units.values() if u["status"] == "unasked"]
        if not candidates:
        ...
        def score(u: dict) -> float:
            s = _CATEGORY_PRIORITY.get(u["category"], 0) * 10.0
            if u["category"] not in covered_categories:
                s += 100.0  # coverage sweep: an untouched category always wins first
            if u["priority_boost"]:
                s += 2.0
            if last_category is not None and u["category"] != last_category:
                s += 1.0
            s += random.random() * 0.5
            return s

        scored = sorted(((u, score(u)) for u in candidates), key=lambda x: x[1], reverse=True)
        chosen, chosen_score = scored[0]

        logger.info(
            "[TopicPool] SELECTED: category=%s source_type=%s source_id=%r "
            "source_field=%s reason=%s score=%.2f",
            chosen["category"], chosen["source_type"], chosen["source_id"],
            chosen["source_field"], chosen["reason"], chosen_score,
        )
        chosen_tier = _CATEGORY_PRIORITY.get(chosen["category"], 0)
        for u, s in scored[1:]:
            reasons = []
            u_tier = _CATEGORY_PRIORITY.get(u["category"], 0)
            if u["category"] in covered_categories and chosen["category"] not in covered_categories:
                reasons.append(f"{chosen['category']} hasn't been covered yet this session (coverage sweep)")
            elif u_tier < chosen_tier:
                reasons.append(f"lower priority tier ({u['category']}={u_tier} < {chosen['category']}={chosen_tier})")
            if last_category is not None and u["category"] == last_category:
                reasons.append("same category as the previous question (diversity penalty applied)")
            if chosen["priority_boost"] and not u["priority_boost"]:
                reasons.append("not weak-area boosted")
            if not reasons:
                reasons.append("lower tie-break score within the same tier")
            logger.info(
                "[TopicPool]   not selected: category=%s source_type=%s source_id=%r score=%.2f — %s",
                u["category"], u["source_type"], u["source_id"], s, "; ".join(reasons),
            )
    ...
    def remaining(self) -> int:
        return sum(1 for u in self.units.values() if u["status"] == "unasked")
